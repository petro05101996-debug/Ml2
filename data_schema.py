...
CANONICAL_FIELDS: List[CanonicalField] = [
    ...
    CanonicalField("price", ["price", "unit_price", "sale_price", "selling_price"], required=True, dtype="float", description="Фактическая цена продажи за единицу"),
    ...
    CanonicalField("cost", ["cost", "cogs", "unit_cost", "purchase_cost"], required=False, dtype="float", description="Себестоимость за единицу"),
    CanonicalField("discount_rate", ["discount_rate", "discount", "discount_pct", "discount_percent", "promo_discount_rate"], required=False, dtype="float", description="Скидка в долях 0..1"),
    CanonicalField("discount_amount", ["discount_amount", "discount_value"], required=False, dtype="float", description="Сумма скидки в валюте, не используется напрямую в расчётах ядра v1"),
    CanonicalField("freight_value", ["freight", "shipping", "shipping_cost", "freight_value", "delivery_cost"], required=False, dtype="float", description="Логистика на единицу"),
