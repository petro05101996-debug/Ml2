...
CANONICAL_FIELDS: List[CanonicalField] = [
    ...
    CanonicalField("quantity", ["quantity", "qty", "units", "units_sold"], required=False, dtype="float", description="Количество", role="target", aggregation="sum", model_eligible=False),
    CanonicalField("revenue", ["revenue", "gmv", "sales_amount", "turnover", "sales_revenue"], required=False, dtype="float", description="Выручка", role="economics_only", aggregation="sum", model_eligible=False),
