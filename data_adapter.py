...
def normalize_transactions(df: pd.DataFrame, mapping: Dict[str, Optional[str]]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if "discount_rate" not in out.columns and "discount" in out.columns:
        out["discount_rate"] = pd.to_numeric(out["discount"], errors="coerce")
    out["discount_rate"] = pd.to_numeric(out.get("discount_rate", 0.0), errors="coerce").fillna(0.0).clip(0.0, 0.95)
    ...
    out["revenue"] = pd.to_numeric(out["revenue"], errors="coerce").fillna(out["price"] * out["quantity"])

    if "cost" not in out.columns:
        out["cost"] = out["price"] * 0.65
    out["cost"] = pd.to_numeric(out["cost"], errors="coerce").fillna(out["price"] * 0.65)

    # canonical rating/review_score: keep only review_score as canonical field
    if "review_score" not in out.columns and "rating" in out.columns:
        out["review_score"] = pd.to_numeric(out["rating"], errors="coerce")
    out["review_score"] = pd.to_numeric(out.get("review_score", np.nan), errors="coerce")

    out["reviews_count"] = pd.to_numeric(out.get("reviews_count", 0.0), errors="coerce").fillna(0.0)
    out["freight_value"] = pd.to_numeric(out.get("freight_value", 0.0), errors="coerce").fillna(0.0)
    out["stock"] = pd.to_numeric(out.get("stock", 0.0), errors="coerce").fillna(0.0)
    out["promotion"] = pd.to_numeric(out.get("promotion", 0.0), errors="coerce").fillna(0.0)

    for c in ["category", "region", "channel", "segment"]:
        out[c] = out.get(c, "unknown")
        out[c] = out[c].fillna("unknown").astype(str)
    out["product_id"] = out["product_id"].astype(str)

    base_known_cols = {
        "date", "product_id", "category", "quantity", "revenue", "price", "cost", "discount_rate", "discount",
    ...
    # prevent duplicate review score user factors
    for c in list(out.columns):
        if c in {f"{USER_FACTOR_NUM_PREFIX}review_score", f"{USER_FACTOR_CAT_PREFIX}review_score"}:
...
def build_daily_panel_from_transactions(txn: pd.DataFrame) -> pd.DataFrame:
    ...
    df["quantity"] = pd.to_numeric(df.get("quantity", 0.0), errors="coerce").fillna(0.0)
    df["revenue"] = pd.to_numeric(df.get("revenue", 0.0), errors="coerce").fillna(0.0)

    num_mean_cols = [
        "price", "cost", "discount_rate", "freight_value", "stock", "promotion", "review_score", "reviews_count"
    ] + [c for c in df.columns if c.startswith(USER_FACTOR_NUM_PREFIX)]
    ...
    for c in cat_mode_cols:
        agg_map[c] = (c, _mode_or_unknown)

    panel = df.groupby([pd.Grouper(key="date", freq="D"), "product_id", "category"], dropna=False).agg(**agg_map).reset_index()

    # fill gaps by product_id across each existing date range
    pieces = []
    for _, g in panel.groupby("product_id", dropna=False):
        ...
        for c in cat_mode_cols:
            if c not in m.columns:
                m[c] = np.nan
            m[c] = m[c].astype(object).ffill().bfill().fillna("unknown").astype(str)
        pieces.append(m)

    out = pd.concat(pieces, ignore_index=True) if pieces else panel
    out["discount"] = pd.to_numeric(out.get("discount_rate", 0.0), errors="coerce").fillna(0.0)
