...
def simulate_v1_horizon_profit(
    ...
    weekday_factors: Optional[Dict[int, float]] = None,
) -> Dict[str, Any]:
    ...
    fc = recursive_v1_demand_forecast(trained_models, base_history, future_dates_df, local_ctx, feature_spec, calibration_factor=float(calibration_factor), forecast_mode=forecast_mode, weekday_factors=weekday_factors)
    daily = fc.copy()
    ...
    baseline_sim = recursive_v1_demand_forecast(trained_models, base_history, future_dates_df, dict(base_ctx), feature_spec, calibration_factor=float(calibration_factor), forecast_mode=forecast_mode, weekday_factors=weekday_factors)
...
def recommend_v1_price_horizon(base_row: Dict[str, Any], trained_models: Dict[str, Any], base_history: pd.DataFrame, base_ctx: Dict[str, Any], feature_spec: Dict[str, Any], n_days: int = 30, objective_mode: str = "maximize_profit", risk_lambda: float = 0.7, can_recommend: bool = True, price_signal_ok: bool = True, calibration_factor: float = 1.0, forecast_mode: str = "strong_signal", weekday_factors: Optional[Dict[int, float]] = None) -> Dict[str, Any]:
    ...
    results = []
    for p in grid:
        sim = simulate_v1_horizon_profit(base_row, float(p), future_dates_df, trained_models, base_history, base_ctx, feature_spec, risk_lambda=float(risk_lambda), calibration_factor=float(calibration_factor), forecast_mode=forecast_mode, weekday_factors=weekday_factors)
