...
def _run_local_simulation(
    ...
    sim = simulate_v1_horizon_profit(
        ...
        float(trained_bundle["pooled_elasticity"]),
        trained_bundle["feature_spec"],
        risk_lambda=float(trained_bundle.get("risk_lambda", 0.7)),
    )
    daily = sim["daily"].copy()
    daily["baseline_sales"] = daily["baseline_sales"] * float(demand_multiplier)
...
def run_v1_what_if_projection(
    ...
    scenario_factors = {k: v for k, v in scenario_factors.items() if k in ALLOWED_SCENARIO_FIELDS}
    ...
    base_ctx["discount"] = sanitize_discount(float(scenario_factors.get("discount", base_ctx.get("discount", 0.0))) * float(discount_multiplier))
    base_ctx["cost"] = sanitize_non_negative(float(scenario_factors.get("cost", base_ctx.get("cost", base_ctx["price"] * 0.65))) * float(cost_multiplier))
    ...
    base_ctx["freight_value"] = sanitize_non_negative(float(scenario_factors.get("freight_value", base_ctx.get("freight_value", 0.0))) * float(freight_multiplier))
    base_ctx["review_score"] = float(scenario_factors.get("review_score", base_ctx.get("review_score", 4.5)))
    ...
    if include_sensitivity:
        for factor, step in SENSITIVITY_STEPS.items():
            ...
            current = float(base_ctx.get(factor, 0.0))
            plus_ctx[factor] = current * (1.0 + step) if abs(current) > 1e-9 else step
    ...
    return {
        ...
        "sanity_warnings": sim.get("sanity_warnings", []) + local.get("econ_warnings", []),
