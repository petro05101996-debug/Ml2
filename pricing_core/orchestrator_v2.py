...
def run_full_pricing_analysis_v2(normalized_txn: pd.DataFrame, target_category: str, target_sku: str, horizon_days: int = 30, scenario_overrides: Dict[str, Any] | None = None, shocks: List[Dict[str, Any]] | None = None) -> Dict[str, Any]:
    ...
    baseline_feature_spec_bt = derive_baseline_feature_spec(panel_train)
    baseline_feature_spec_final = derive_baseline_feature_spec(panel_features_baseline)
    ...
    if not tiny_mode:
        trained_baseline_bt = train_baseline_model(panel_train, baseline_feature_spec_bt, small_mode=len(panel_train) < 200, training_profile="backtest")
        trained_baseline_final = train_baseline_model(panel_features_baseline, baseline_feature_spec_final, small_mode=len(panel_features_baseline) < 200, training_profile="final")

        base_ctx = {c: (target_history[c].dropna().astype(str).iloc[-1] if c in ["product_id", "category", "region", "channel", "segment"] and c in target_history else "unknown") for c in baseline_feature_spec_final.get("baseline_context_features", [])}
        baseline_forecast = recursive_baseline_forecast(trained_baseline_final, target_history, future_dates, base_ctx, baseline_feature_spec_final)
        ...
        baseline_rolling = {"rolling_diag": pd.DataFrame(), "rolling_metrics": pd.DataFrame(), "rolling_summary": {"n_valid_windows": 0, "median_wape": np.nan, "median_bias_pct": np.nan, "median_sum_ratio": np.nan, "max_wape": np.nan}}
        baseline_oof = target_train[["date", "sales"]].copy()
        baseline_oof["baseline_oof"] = np.nan

    factor_train = target_train[[c for c in target_train.columns if c != "baseline_oof"]].merge(baseline_oof[["date", "baseline_oof"]], on="date", how="left")
    factor_feature_spec = derive_factor_feature_spec(factor_train)
    factor_train = build_factor_feature_matrix(factor_train, factor_feature_spec)
    factor_train = factor_train[factor_train["baseline_oof"].notna()].copy()
    factor_train["factor_target"] = build_factor_target(factor_train)

    cont = factor_feature_spec.get("controllable_features", [])
    variative = [c for c in cont if c in factor_train.columns and pd.to_numeric(factor_train[c], errors="coerce").nunique(dropna=True) > 1]
    price_unique_count = int(pd.to_numeric(factor_train.get("price", np.nan), errors="coerce").nunique(dropna=True))
    factor_trainable = len(factor_train) >= 60 and len(variative) > 0 and (price_unique_count >= 4 or len([c for c in variative if c != "price"]) > 0)
    ...
    if factor_trainable:
        ...
        factor_backtest.update({
            ...
            "n_variative_controllable_features": int(len(variative)),
            "price_unique_count": int(price_unique_count),
        })
    else:
        trained_factor = None
        factor_backtest = {
            "trained": False,
            "reason": "factor signal insufficient",
            "n_train_rows": int(len(factor_train)),
            "n_valid_windows": 0,
            "n_variative_controllable_features": int(len(variative)),
            "price_unique_count": int(price_unique_count),
            "price_sign_stability": 0.0,
            "factor_ood_share": 0.0,
        }
    ...
    scenario_result = run_scenario_forecast(
        ...
        baseline_feature_spec=baseline_feature_spec_final,
        factor_feature_spec=factor_feature_spec if factor_trainable else None,
    ...
    factor_contributions = compute_factor_contributions(target_history, future_dates, base_ctx, scn_ctx, trained_factor, factor_feature_spec if factor_trainable else {"controllable_features": []}) if factor_trainable else pd.DataFrame(columns=["factor_name", "contribution_abs", "contribution_pct", "confidence", "note"])
    ...
    bundle = build_model_bundle(
        ...
        trained_factor=trained_factor,
        baseline_feature_spec_bt=baseline_feature_spec_bt,
        baseline_feature_spec_final=baseline_feature_spec_final,
        ...
        warnings=scenario_result.get("warnings", []) + scenario_result.get("ood_flags", []),
